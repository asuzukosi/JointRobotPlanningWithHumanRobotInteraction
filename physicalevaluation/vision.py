...
# create dataclass to store location object
@dataclass
class Location:
    x: float
    y: float
...
# def build_pipeline():
#     # Builds an intelrealsense camera pipeline
#     pipeline = rs.pipeline()
#     config = rs.config()
...
def get_camera_image():
    """
    Get top view image from camera in robot setup
    """
    # Configure depth and color streams
...
def SceneDescription(image):
    """
    Use gpt4 vision API to make scene description
    """
    import base64
    import requests

    # OpenAI API Key
    api_key = "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"
