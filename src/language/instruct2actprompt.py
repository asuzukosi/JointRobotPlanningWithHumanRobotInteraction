...
# NOTE: the word in one instruction inside {} stands for the visual part of the
    instruction and will be obtained with get operation
...
def ImageCrop(image: Image.Image, masks: dict):
    ...
    """
    pass
def CLIPRetrieval(objs: list, query: str | Image.Image , pre_obj1: int = None,
    pre_obj2: int = None) -> np.ndarray:
    """Retrieve the desired object(s) with CLIP, the query could be string or an
    image
Examples:
    obj_0 = CLIPRetrieval(objs=objs, query='the yellow and purple polka dot
        pan') # the query is a string
...
def get_objs_match(objs_list1: list, objs_list2: list) -> tuple:
    """Get correspondences of objects between two lists using the Hungarian
        Algorithm"""
    return (list, list)
...
def Pixel2Loc(obj: np.ndarray, masks: np.ndarray) -> np.ndarray:
    ...
    pass
def PickPlace(pick: np.ndarray, place: np.ndarray, bounds: np.ndarray,
    yaw_angle_degree: float = None, tool: str = "suction") -> str:
    """Pick and place the object based on given locations and bounds"""
pass
def DistractorActions(mask_obs: list, obj_list: list, tool: str = "suction") ->
    list:
    """Remove observed objects that conflict with the goal object list"""
pass
def RearrangeActions(pick_masks: list, place_masks: list, pick_ind: list,
    place_ind: list, bounds: np.ndarray, tool: str = "suction") -> list:
...
def main_1() -> dict:
    """Execute the given instructions of placing the checkerboard round into the
        yellow and purple polka dot pan"""
    image = GetObsImage(obs)
    masks = SAM(image=image)
    objs, masks = ImageCrop(image=image, masks=masks)
...
def main_3() -> dict:
    """Execute the given instructions of rearranging the objects to match the
        objects in the given scene"""
    ...
    action_2 = RearrangeActions(pick_masks=masks_obs, place_masks=masks_goal,
        pick_ind=col, place_ind=row, bounds=BOUNDS)
    action_3 = RearrangeActions(pick_masks=masks_goal, place_masks=masks_obs,
        pick_ind=row, place_ind=col, bounds=BOUNDS)
    actions = []
    actions.extend(action_1).extend(action_2).extend(action_3)
    info = RobotExecution(action=actions)
    return info
## Example 4
# Instruction: Put the yellow and blue stripe object in {scene} into the orange
object.
def main_4() -> dict:
    """Execute the given instructions of placing the yellow and blue stripe object
        in scene into the orange object"""
    image = GetObsImage(obs)
    masks_obs = SAM(image=image)
    objs_goal, masks_goal = ImageCrop(image=templates['scene'],
        masks=SAM(image=templates['scene']))
    ...
    return info
## Example 5
# Instruction: Put the {dragged_obj} into the {base_obj_1} then {base_obj_2}.
...
def mian_5() -> dict:
    ...
    dragged_obj = CLIPRetrieval(objs, templates['dragged_obj'],
        pre_obj1=base_obj_1, pre_obj2=base_obj_2)
...
------
Please solve the following instruction step-by-step. You should implement the
    main() function and output in the Python-code style.
